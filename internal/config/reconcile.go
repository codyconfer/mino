...
func LoadConfigAndDirectives(homeOverride, configFile string, interactive bool, in io.Reader, out io.Writer) (*Config, *Directives, *sisyphus.Manager, error) {
	...
	cfg, err := reconcileConfig(home, homeOverride, configFile, mgr, res)
	if err != nil {
		return nil, nil, nil, err
	}
		...
		if q, err = reconcileCollection(mgr, res, home, DirQueries); err == nil {
			if f, err = reconcileCollection(mgr, res, home, DirFilters); err == nil {
				if fl, err = reconcileCollection(mgr, res, home, DirFlights); err == nil {
					if r, err = reconcileCollection(mgr, res, home, DirRoles); err == nil {
						directives, err = NewDirectives(q, f, fl, r)
					}
				}
			}
		}
...
func reconcileConfig(home, homeOverride, configFile string, mgr *sisyphus.Manager, res sisyphus.Resolver) (*Config, error) {
	if configFile != "" {
...
func (r *Resolver) Resolve(rec sisyphus.Reconciliation) (sisyphus.Action, error) {
	...
	if !r.interactive {
		if r.preferDB && rec.HasDB {
			log.Warnf("%s differs from DuckDB; using DuckDB (prefer_duckdb)", rec.Name)
			return sisyphus.ActionUseDB, nil
		}
		log.Warnf("%s differs from DuckDB; using the file for this session (not persisted)", rec.Name)
...
func printReconcileFile(out io.Writer, rec sisyphus.Reconciliation) {
