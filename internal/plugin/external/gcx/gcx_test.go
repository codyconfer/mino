...
type memTokens map[string]auth.Credential

func (m memTokens) Get(_ context.Context, service string) (auth.Credential, bool, error) {
	c, ok := m[service]
	return c, ok, nil
}
...
func TestFetchAuthAndContext(t *testing.T) {
	...
	tok := memTokens{TokenKey: {AccessToken: "glsa_test", Scope: "irm"}}
...
func TestStubActions(t *testing.T) {
	...
	err := acts[0].Run(context.Background(), nil)
	if err == nil || errs.KindOf(err) != errs.KindConfig {
		t.Fatalf("expected config stub error, got %v", err)
	}
	err = acts[1].Run(context.Background(), nil)
	if err == nil || errs.KindOf(err) != errs.KindConfig {
		t.Fatalf("expected config stub error for add-activity, got %v", err)
	}
