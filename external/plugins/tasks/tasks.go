...
func New(ga googleauth.Auth, lists []string, showCompleted bool, max int) plugin.Query {
	return &gtasksSignal{auth: ga, lists: lists, showCompleted: showCompleted, max: max}
}

func (s *gtasksSignal) Name() string { return "tasks" }

func (s *gtasksSignal) Fetch(ctx context.Context) ([]plugin.Section, error) {
	...
	for _, tl := range lists.Items {
		...
		sec := plugin.Section{Signal: "tasks", Title: "Tasks: " + tl.Title}
...
func CreateTask(ctx context.Context, ga googleauth.Auth, listRef, title, notes, due string) (plugin.Item, error) {
	...
	if err != nil {
		return plugin.Item{}, errx.Wrapf(err, "tasks: creating task in %q", listTitle)
	}
...
func resolveList(ctx context.Context, svc *tasksapi.Service, ref string) (id, title string, err error) {
	lists, err := svc.Tasklists.List().MaxResults(maxResults).Context(ctx).Do()
	if err != nil {
		return "", "", errx.Wrap(err, "tasks: listing task lists")
	}
	var names []string
	for _, tl := range lists.Items {
	...
	return "", "", errx.Newf("tasks: task list %q not found", ref).
		WithHint("available lists: %s", strings.Join(names, ", "))
}

func newService(ctx context.Context, ga googleauth.Auth) (*tasksapi.Service, error) {
	return googleauth.Service(ctx, ga, "tasks", []string{tasksapi.TasksScope}, tasksapi.NewService)
}

func taskToItem(t *tasksapi.Task, listTitle string) plugin.Item {
	var ts time.Time
	switch {
	case t.Due != "":
...
func normalizeDue(due string) (string, error) {
	if len(due) == 10 {
		if _, err := time.Parse("2006-01-02", due); err != nil {
			return "", errx.Newf("invalid due date %q", due).
